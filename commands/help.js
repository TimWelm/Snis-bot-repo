...
module.exports.run = async (bot, message, args) => {
  ...
  if(config.Staff_Help_Menu == `true`) {
    ...
    if(!message.member.roles.has(667436678473842727)) return message.channel.send(Embed({title: '**STAFF COMMANDS**', thumbnail: `${bot.user.displayAvatarURL}`, description: Object.values(adminHelp).map(h => `**${h.usage.replace(/\-/g, require("../data/prefixes.json")[message.guild.id].prefix)}** ${h.description}`).join("\n")}));
  }
}
module.exports.help = {
  name: "help"
